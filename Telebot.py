...
import pytz
import re
import Linear_RG_for_Corona as CoLR
import sys
...
def Corona_update():
    ...
    '''
    #그냥 로드할때 해도 될듯 manual한 update할 일이 없다.
    infected = infected[-5:-4]+infected[-3:] # 만명 넘으면 업데이트 해야함!!!
    updator = CoLR.Updator()
    updator(int(infected))
    '''
...
def Newbie_checker(chat_id): #알고리즘 수정 필요 회원 list에 없을때 추가하는 방식으로
    user_list = []
    r_user_list_stream = open('user_list.csv', 'r', encoding='utf-8')
    user_list_file = csv.reader(r_user_list_stream)
    for line in user_list_file:
        user_list.append(line)
    r_user_list_stream.close()
    if chat_id in user_list:
        pass
    else:
        w_user_list_stream = open('user_list.csv', 'a',newline='', encoding='utf-8')
        user_list.append(chat_id)
        wr = csv.writer(w_user_list_stream)
        wr.writerow(chat_id)
        w_user_list_stream.close()
